    ...
    public function handle()
    {
        ...
        foreach($dataImport as $item){
            if(isset($item['siret'])){
                ...
                ->orWhere('region_code',strlen($item['nom_de_la_region']) == 1 ? '0'.$item['nom_de_la_region'] : $item['nom_de_la_region'])
                ->orWhere('name_region',$item['nom_de_la_region'])
                ->first();
                $depart=Departement::where('departement_code',$item['code_du_departement'])
                ->orWhere('departement_code', strlen($item['code_du_departement']) == 1 ? '0'.$item['code_du_departement'] : $item['code_du_departement'])
